...
# array_states = np.zeros((3219,27))
array_states = np.zeros((3219,15))
    ...
    for g in range(1,4):
        ...
        if days_animal.count() != 0:
            ...
            for d in days_for_animal:
                n_group = df[(df['ANIMAL_NUMBER']==a) & (df['GROUP_NUMBER']==g) & (df['DAYS']==d)]['DURATION'].sum()
                ...
                if n_group >= tol_time:
                    ...
                    for n in range(2,14):
                        ...
                        # array_states[n_a,n] = df[(df['ANIMAL_NUMBER']==a) & (df['GROUP_NUMBER']==g) & (df['DAYS']==d) & (df['STATE_NUMBER']==n-1)]['STATE_NUMBER'].count()
                        # array_states[n_a,n+12] = df[(df['ANIMAL_NUMBER']==a) & (df['GROUP_NUMBER']==g) & (df['DAYS']==d) & (df['STATE_NUMBER']==n-1)]['DURATION'].sum()
...
states.to_csv('Dates/states_for_days.csv')
states.to_excel('Dates/excel/states_for_days.xlsx')