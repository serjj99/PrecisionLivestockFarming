...
df = pd.read_csv('Dates/Dates_times/2/states_times_animal_1542.csv', index_col='Unnamed: 0')
...
n_states = int(max(df['STATE_NUMBER']))
...
y_actual_CAT = kr.utils.to_categorical(y_actual-1, num_classes=n_states)
y_to_predict_CAT = kr.utils.to_categorical(y_to_predict-1, num_classes=n_states)
...
X_to_predict = X_to_predict.reshape(1, n_lookback, n_states)
...
model.add(LSTM(units=128, dropout=0.2, return_sequences=True, input_shape=(n_lookback, n_states)))  #input_shape toma una tupla de dos valores que definen la cantidad de pasos de tiempo y características
model.add(Dense(64, activation='relu'))
model.add(Dense(n_states, activation='softmax')) #n_forecast

model.compile(loss='categorical_crossentropy', optimizer='adam', metrics = 'categorical_accuracy')
history = model.fit(X, Y, epochs=3, batch_size=32, verbose=0, validation_data=(X_to_predict, y_to_predict_CAT.reshape(1, n_forecast, n_states)))
...
Y_predict = model.predict(y_to_predict_CAT.reshape(1, n_forecast, n_states))
...
plt.scatter(minutes_to_predict, Y_predict, color='red', label='Estados predecidos')
